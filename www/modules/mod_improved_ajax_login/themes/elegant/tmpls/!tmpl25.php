...
<?php if ($guest): // LOGIN ?>
  <?php if (@$module->view != 'reg'): ?>
    <?php if ($loginpopup): ?>
      <a class="logBtn selectBtn" onclick="return false" > <!--href="<?php echo JRoute::_('index.php?option=com_users&view=login') ?>"-->
        ...
        <?php else: ?>
          <span class="loginBtn"><img src="<?php echo JRoute::_("{$module->url}/themes/elegant/images/blocks.png")?>" /> <?php echo JText::_('IAL_JLOGIN') ?></span>
        <?php endif ?>
      </a>
  ...
  <?php endif ?>
    ...
    <?php else: ?>
      <a class="regBtn selectBtn <?php if (!$loginpopup) echo 'fullWidth ' ?>"  <?php if (!$loginpopup) echo 'href="'.$regpage.'"' ?>>  <!--href="<?php echo $regpage ?>"-->
    ...
    <div class="loginWndInside">
      <div class="ial-arrow-up">
        <div style="position:relative">
			...
			<button class="ial-close loginBtn"><img src="<?php echo JRoute::_("modules/{$module->module}/themes/elegant/images/x.png")?>" alt="x" width="8" height="10"/></button>
      <?php if ($allowUserRegistration && @$module->view != 'reg' && ($regp[0] == 'joomla' || isset($_SESSION['oauth']['twitter']))) 
              require dirname(__FILE__).'/reg25.php' // REGISTER FORM ?>
    </div>
	</div>
...
<?php if ($loginpopup) : ?>
  <div><span class="headerPrefixTel">(3412)</span><span class="headerTel">63-98-93</span></div>
<?php endif; ?>
</div>