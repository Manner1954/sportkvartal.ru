...
for ($a = 1; $a <= 30; $a++) {
  if ((($this->item->id == $a*5) or ($this->item->ordering == $a+3)) AND $this->item->hits >='100' ) {}} ?>
 	...
 	<header<?php if (isset($images->image_fulltext) and !empty($images->image_fulltext)) : ?> class="itemHasImage gkPage"<?php endif; ?> class="gkPage">
		<?php if ($params->get('show_page_heading', 1)) : ?>
		<?php if($this->escape($params->get('page_heading')) <> "") { echo "<h1>"; echo $this->escape($params->get('page_heading')); echo "</h1>";} ?>
		<?php endif; ?>

		<?php if (($params->get('show_create_date')) || ($params->get('show_modify_date')) || $canEdit || $params->get('show_print_icon') || $params->get('show_email_icon') || $params->get('show_parent_category') && $this->item->parent_slug != '1:root' || $params->get('show_category') || $params->get('show_hits')) : ?>
		<ul>	
		...
		<h2>
			...
			<?php else : ?>
				<div class="headerBorderRadius"> <?php echo $this->escape($this->item->title); ?> </div>
			<?php endif; ?>
		</h2>
		<?php endif; ?>
