...
class EventBookingHelper {
	...
	function acceptRegistration($eventId) {
		...
		if ($row->cut_off_date == $db->getNullDate()) {
			// ARt $sql = 'SELECT DATEDIFF(NOW(), event_date) AS number_days FROM #__eb_events WHERE id='.$eventId ;
      // +++ ARt
      if ($row->recurring_end_date == $db->getNullDate()) {
          //$sql = 'SELECT COUNT(event_date) AS number_days FROM #__eb_events WHERE id='.$eventId.' AND NOW() BETWEEN event_date AND event_end_date';
          $sql = 'SELECT COUNT(event_date) AS number_days FROM #__eb_events WHERE id='.$eventId.' AND NOW() <= event_end_date';  
      } else {
          //$sql = 'SELECT COUNT(event_date) AS number_days FROM #__eb_events WHERE id='.$eventId.' AND NOW() BETWEEN event_date AND recurring_end_date';
          $sql = 'SELECT COUNT(event_date) AS number_days FROM #__eb_events WHERE id='.$eventId.' AND NOW() <= recurring_end_date';
      }	
      // --- ARt				
		} else {
	...
	function getDailyRecurringEventDates($startDate, $endDate, $dailyFrequency, $numberOccurencies) {
		...
		if ($numberOccurencies) {
			$count = 1 ;
			//$i = 1 ;
			while ($count < $dailyFrequency) {
				//$i++ ;
				//$nextEventDate = $startTime + ($i-1)*$dailyFrequency*24*3600 ;
        $nextEventDate = $startTime + $count*24*3600 ;
        if($count <= $numberOccurencies) {							
    ...
    //stop();
      ...
      $weekCount = 0 ;
			$i = 0 ;
			$weekDay =  date('w', $startTime) ;                 // РќРѕРјРµСЂ РґРЅСЏ РЅРµРґРµР»Рё РґР°С‚С‹ РЅР°С‡Р°Р»Р° СЃРѕР±С‹С‚РёСЏ
			$startTimeWeek = $startTime - $weekDay*24*3600 ;    // РќР°С‡Р°Р»СЊРЅР°СЏ РґР°С‚Р° РЅРµРґРµР»Рё (РЅР°С‡Р°Р»Рѕ СЂР°Р±РѕС‚С‹ СЃРѕР±С‹С‚РёСЏ)
      ...
      while ($weekCount <= $weeklyFrequency) {
        ...
        $startTimeWeek = $startTimeWeek + (7*24*3600);    //РќР°С‡Р°Р»Рѕ СЃР»РµРґСѓСЋС‰РµР№ РЅРµРґРµР»Рё  
			}						
		} else {
