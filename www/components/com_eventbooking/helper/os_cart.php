...
class EBCart {				
	...
	function getEvents() {
		...
		if (count($items)) {
			$sql = 'SELECT a.*, SUM(b.number_registrants) AS total_registrants  FROM #__eb_events AS a LEFT JOIN #__eb_registrants AS b ON (a.id = b.event_id AND b.group_id=0 AND (b.published=1 OR b.payment_method="os_offline")) WHERE a.id IN ('.implode(',', $items).') GROUP BY a.id';
			$db->setQuery($sql) ;				
