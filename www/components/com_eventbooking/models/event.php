...
{		
	...
	function getData() {
		if (empty($this->_data)) {
			...
			$sql = 'SELECT SUM(number_registrants) FROM #__eb_registrants WHERE event_id='.$this->_data->id.' AND group_id=0 AND (published = 1 OR (payment_method = "os_offline" AND published != 2))';
			$this->_db->setQuery($sql) ;
			...
			if ($userId > 0) {
				$sql = 'SELECT COUNT(id) FROM #__eb_registrants WHERE user_id='.$userId.' AND event_id='.$id.' AND (published=1 OR (payment_method="os_offline" AND published != 2))';
				$this->_db->setQuery($sql) ;
