	...
	<?php foreach ($this->letters as &$letter): ?>
		<span class="fsf_glossary_letter">
			<?php if ($this->use_letter_bar == 2): ?>
				<a href='<?php echo FSFRoute::x('index.php?option=com_fsf&view=glossary&letter=' . strtolower($letter->letter)); ?>'>&nbsp;<?php echo $letter->letter; ?>&nbsp;</a>
			<?php else : ?>
				<a href='<?php echo FSFRoute::x('index.php?option=com_fsf&view=glossary#letter_' . strtolower($letter->letter)); ?>'>&nbsp;<?php echo $letter->letter; ?>&nbsp;</a>
			<?php endif; ?>
		</span>
	<?php endforeach; ?>
	</div>
